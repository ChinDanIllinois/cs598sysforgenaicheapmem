...
try:
    ...
    pass # Caught by lightmem itself if missing, but we also use it in GeminiModel below
...
class GeminiModel:
    """
    An example Gemini model class for generating responses during the evaluation.
    """
    def __init__(
            self,
            model_name: str,
    ...
    def call(self, messages: List[Dict[str, str]], **kwargs):
        ...
        if self.options:
            if "temperature" in self.options: config_kwargs["temperature"] = self.options["temperature"]
            if "top_p" in self.options: config_kwargs["top_p"] = self.options["top_p"]
            if "top_k" in self.options: config_kwargs["top_k"] = self.options["top_k"]
            if "num_ctx" in self.options: config_kwargs["max_output_tokens"] = self.options["num_ctx"]
            if "seed" in self.options: config_kwargs["seed"] = self.options["seed"]
            # seed argument tracking is somewhat dependent on specific parameters for google api
            # pass them mapped cleanly.
...
def main():
    ...
    llm = GeminiModel(
        ...
        options=your_gemini_options_stable,
    )
    llm_judge = GeminiModel(
        model_name=your_gemini_JUDGE_model_name,
        options=your_gemini_options_stable,
    )
